...
def _log_execution(
    code: str, success: bool, result: Any, error: str | None, duration_ms: float
) -> None:
    ...
    if _script_config["require_consent"]:
        ...
        if not consent_granted:
            return _error(
                ...
                data={
                    "warning": "This will execute arbitrary Python code in your Blender session."
                },
                started=started,
            )
...
def _execute_with_timeout(code: str, timeout: int, bpy: Any) -> dict[str, Any]:
    ...
    if return_value is not None:
        try:
            if hasattr(return_value, "__iter__") and not isinstance(
                ...
                return_value, (bool, int, float, str, list, dict, type(None))
            ):
