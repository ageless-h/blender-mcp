...
GenericCollectionHandler SHALL provide a default `write()` method that iterates the `properties` dict, calls `_set_nested_attr()` for each key-value pair on the looked-up item, and returns `{"name": name, "modified": [list of modified property paths]}`.
...
- **THEN** system looks up the item via `get_item(name)`, applies each property via `_set_nested_attr()`, and returns `{"name": name, "modified": [keys]}`
...
GenericCollectionHandler SHALL provide a default `list_items()` method that iterates the bpy.data collection, calls `_list_fields(item)` for each item, and returns `{"items": [...], "count": N}`.
...
- **THEN** system iterates `get_collection()`, builds a list via `_list_fields()` per item, and returns `{"items": [...], "count": len(items)}`
