# -*- coding: utf-8 -*-
"""Blender script to start the MCP socket server for HEADFUL testing.

This version keeps Blender's GUI open so you can see the scene changes.
...
The server will run until you close Blender.
"""
...
def _scene_write(payload: dict[str, Any], *, started: float) -> dict[str, Any]:
    ...
    if cleanup is None:
        cleanup = False  # Default to False for headful testing so you can see the objects!
    ...
    try:
        ...
        print(f"[Scene Write] Created cube '{name}' at origin (cleanup={cleanup})")
...
def main():
    ...
    print("=" * 60)
    print("Blender MCP Socket Server (HEADFUL MODE)")
    print("=" * 60)
    ...
    if result.get("ok"):
        ...
        print("\n🎨 HEADFUL MODE - You can see objects created in the viewport!")
        print("\nServer is ready to accept capability execution requests.")
        print("\nTo test, run in another terminal:")
        print('  python test_socket_connection.py')
        print("\n" + "=" * 60)
        print("\n📝 Server will run until you close Blender.")
        print("   Watch the Blender Console for request logs.\n")
    else:
